// Variables globales
        ...
        window.showScreen = function(screenId) {
            ...
            // Iniciar o pausar el motor 3D
            if (screenId === 'game-screen') {
                window.isGameRunning = true;
                if(window.initMultiplayerGame) window.initMultiplayerGame();
        ...
        window.changeQuality = function(direction) {
        ...
        window.changeThemeCarousel = function(direction) {
        ...
        // Avatar
        window.selectAvatar = function(cardElement, colorHex) {
            clickSound();
            const cards = document.querySelectorAll('.avatar-card');
            cards.forEach(card => card.classList.remove('selected'));
            cardElement.classList.add('selected');
            window.playerColor = colorHex; // Guardamos el color para Three.js
        };
